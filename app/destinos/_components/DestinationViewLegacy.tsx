...
/** Rama antigua: 1 única próxima salida. */
export default async function DestinationViewLegacy({ slug }: { slug: string }) {
  ...
  const nextHome = futureHomes[0] || destination.Homes[0] || null;
  const departure = nextHome ? formatDeparture(nextHome.checkInTime) : null;
  const available = nextHome ? calculateAvailableSeats(nextHome) : 0;
